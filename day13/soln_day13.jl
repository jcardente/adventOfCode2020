# ------------------------------------------------------------
# Solution to Advent of Code 2020 Puzzle Day 13
#
# PUZZLE:
#
...
# port. After that, each bus travels to the airport, then various
# other locations, and finally returns to the sea port to repeat its
# journey forever.
# 
...
# on. If you are there when the bus departs, you can ride that bus to
# the airport!
# 
...
# earliest bus you can take to the airport. (There will be exactly one
# such bus.)
#
...
# IDs in service are 7, 13, 59, 31, and 19. Near timestamp 939, these
# bus IDs depart at the times marked D:
#
# Here, the earliest timestamp you could depart is 939, and the bus
# IDs in service are 7, 13, 59, 31, and 19. Near timestamp 939, these
# bus IDs depart at the times marked D:
...
function inputLoad(fname)
  ...
  busids = String[]
  open(fname, "r") do f
    tstamp = parse(Int64,readline(f))
    busids = [string(s) for s in split(readline(f),",")]
  end
  return tstamp, busids
end
...
function main()  
  ...
  tstamp, busids = inputLoad(args["input"]);

  if (args["part"] == "part1")
    busids  = [parse(Int, x) for x in filter(id -> id != "x", busids)]
