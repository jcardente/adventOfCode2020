# ------------------------------------------------------------
# Solution to Advent of Code 2020 Puzzle Day XX
#
# PUZZLE:
#
# --- Day 22: Crab Combat ---
...
# card is above the other card. If this causes a player to have all of
...
# move to the bottom of player 1's deck such that 9 is above 5. In
...
# score. The bottom card in their deck is worth the value of the card
# multiplied by 1, the second-from-the-bottom card is worth the value
# of the card multiplied by 2, and so on. With 10 cards, the top card
# is worth the value on the card multiplied by 10. In this example,
...
# (you offer to play with the same starting decks as before - it's
# only fair). Then, the game consists of a series of rounds with a few
# changes:
# 
...
# beginning of the round and places them on the bottom of their own
# deck (again so that the winner's card is above the other card). Note
# that the winner's card might be the lower-valued of the two cards if
# they won the round due to winning a sub-game. If collecting cards by
# winning the round causes a player to have all of the cards, they
# win, and the game ends.
# 
# Here is an example of a small game that would loop forever without
# the infinite game prevention rule:
# 
# Player 1:
# 43
# 19
...
# has at least 3 cards left and player 2 has at least 7 cards left,
...
# trigger the sub-game). During this sub-game, the game that triggered
# it is on hold and completely unaffected; no cards are removed from
# players' decks to form the sub-game. (For example, if player 1 drew
# the 3 card, their deck in the sub-game would be copies of the next
...
# cards they have in their original deck using the same rules as
# regular Combat. In the above game, the winning player's score is
# 291.
# 
# Defend your honor as Raft Captain by playing the small crab in a
# game of Recursive Combat using the same two decks as before. What is
...
function inputLoad(fname)
  input = read(fname, String)

  deckStrings = [strip(replace(s, "\n"=> " ")) for s in split(input,"\n\n")]
  deckStrings = [strip(replace(s, r"Player \d:" => "")) for s in deckStrings]
  decks = [[parse(Int,d) for d in split(ds, ' ')] for ds in deckStrings]
...
function handSignature(d1,d2)
  return (join([string(c) for c in d1], ":") *
          "=" *
          join([string(c) for c in d2], ":"))
end
...
function main()  
  ...
  decks = inputLoad(args["input"])

  if args["part"] == "part1"
    winner, d1, d2 = playGameP1(decks[1], decks[2])

  else
    winner, d1, d2 = playGameP2(decks[1], decks[2])
    
  end

  @printf("Player %d won\n", winner)
  @printf("Player 1 deck: %s\n", join([string(c) for c in d1], ", "))
  @printf("Player 2 deck: %s\n", join([string(c) for c in d2], ", "))
    
  windeck = length(d1) > 0 ? d1 : d2
  coef    = Array(reverse(1:length(windeck)))
  answer  = sum(windeck .* coef)
