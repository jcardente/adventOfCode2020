# ------------------------------------------------------------
# Solution to Advent of Code 2020 Puzzle Day 19
#
# PUZZLE:
#
...
function parseInput(lines)
  # Process Rules
  rules = Dict{String, Array{Array{String,1},1}}()
  idx = 1
  while idx < length(lines)
    ...
    m = match(r"^(\d+): (.*)$", l)
    rid = m.captures[1]
    rule = replace(strip(m.captures[2]), "\"" => "")
   
    if length(rule)==1 && isletter(rule[1])
      # Primitive rule just store it
      rules[rid] = [[rule]]
      
    else
      # Composite patterns
      clauses = Array{String,1}[]
      for oc in split(rule, '|')
        push!(clauses, split(strip(oc), ' '))
      end
      rules[rid] = clauses
...
function main()  
  ...
  # NB - implementation works for both parts of the days
  #      puzzle.
  rules, patterns = parseInput(lines);
  passedCount = 0
  for p in patterns
    if checkRule(["0"], rules, p, 1)
      passedCount += 1
    end
  end

  @printf("%d patterns passed\n", passedCount)
end
