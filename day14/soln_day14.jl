# ------------------------------------------------------------
# Solution to Advent of Code 2020 Puzzle Day 14
#
# PUZZLE:
#
# --- Day 14: Docking Data ---
...
# it specifies will overwrite two bits in every written value: the 2s
...
# values left in memory after the initialization program
# completes. (The entire 36-bit address space begins initialized to
# the value 0 at every address.) In the above example, only two values
# in memory are not zero - 101 (at address 7) and 64 (at address 8) -
...
function main()  
  ...
  for l in program
    if startswith(l, "mask")
      m = match(r"^mask = ([X10]{36})", l)
      maskString = m.captures[1];
      oneMask, zeroMask = parseMaskString(maskString);
  ...
  memsum = sum(values(memory))
  @printf("Sum of memory contents: %d\n", memsum)  
