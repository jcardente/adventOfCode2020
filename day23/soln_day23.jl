# ------------------------------------------------------------
# Solution to Advent of Code 2020 Puzzle Day 23
#
# PUZZLE:
#
# --- Day 23: Crab Cups ---
...
# be five cups in the circle; going clockwise around the circle from
# the first cup, the cups would be labeled 3, 2, 4, 1, 5, and then
# back to 3 again.
# 
...
# into a single string with no extra characters; each number except 1
# should appear exactly once. In the above example, after 10 moves,
...
# cups after cup 1?
# 
# Your puzzle input is 496138527.
#
# --- Part Two ---
# 
...
# starting from the number after the highest number in your list and
# proceeding one by one until one million is reached. (For example, if
# your labeling were 54321, the cups would be numbered 5, 4, 3, 2, 1,
# and then start counting up from 6 until one million is reached.) In
# this way, every number from one through one million is used exactly
# once.
# 
...
# if you predict what the labels on those cups will be when the crab
# is finished.
# 
# In the above example (389125467), this would be 934001 and then
# 159792; multiplying these together produces 149245887792.
# 
# Determine which two cups will end up immediately clockwise of cup
# 1. What do you get if you multiply their labels together?
...
function inputLoad(fname)
  ...
  [parse(Int,c) for c in strip(lines[1])]
end


function circleToString(circle, currLabel)
  currcup = currLabel
  cs = String[]
  for i in 1:length(circle)
    push!(cs, currcup == currLabel ? @sprintf("(%d)", currcup) : string(currcup))
    currcup = circle[currcup]
  end
...
function makeCupCircle(cups)
  circle = zeros(Int64, length(cups))
  for i in 1:(length(cups)-1)
    circle[cups[i]] = cups[i+1]
  end
  circle[cups[end]] = cups[1]

  return circle
end


function doMove(circle, currCup, maxCup, verbose)

  if verbose == true
    @printf("cups: %s\n", circleToString(circle, currCup))
  end
  ...
  if verbose == true
    @printf("pick up: %s\n", join([string(c) for c in cupsTake], " "))
  end
...
function playGame(circle::Array{Int64,1}, startCup::Int64, maxCup::Int64,
                  numMoves::Int64, verbose::Bool)
  currCup   = startCup
  moveCount = 1
  while moveCount <= numMoves
    ...
    doMove(circle, currCup, maxCup, verbose)
    currCup = circle[currCup]
    moveCount += 1
  end

  return circle, currCup
end
...
function main()  
  ...
  cups   = inputLoad(args["input"])
  maxCup = max(cups...)

  if args["part"] == "part1"

    circle = makeCupCircle(cups)
    circle, currCup = playGame(circle, first(cups),
                                 maxCup,
                                 100, true)
    println("\n-- final --")
    @printf("cups: %s\n", circleToString(circle, currCup))
    ...
    cups   = vcat(cups, Array((maxCup+1):1_000_000));
    circle = makeCupCircle(cups);
    
    circle, currCup = playGame(circle,
                               first(cups),
                               1_000_000,
                               10_000_000,
                               false);
