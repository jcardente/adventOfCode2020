...
function utilCheckYear(yrstring, yrmin, yrmax)
  if match(r"^\d{4}$", yrstring) != nothing
    year = parse(Int, yrstring)
...
function ruleCheckHeight(pdict)
  hgt = pdict["hgt"]
  m = match(r"(\d{2,3})(cm|in)", hgt)
...
function main()
  ...
  passportStrings = inputLoad(args["input"]);
