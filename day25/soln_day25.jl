# ------------------------------------------------------------
# Solution to Advent of Code 2020 Puzzle Day 25
#
# PUZZLE:
#
...
# this morning; you can take yours now and give them your room deposit
...
# there can reset it for you." Still catching your breath, you
# describe the status of the elevator and the exact number of stairs
# you just had to climb.
# 
...
#   a subject number. The door always uses a different, secret loop
#   size.
# 
...
#   oor's secret loop size. The result is called the door's public
#   key.
# 
...
# the same process, you can determine that the door's loop size is 11,
...
# the subject number of 17807724 (the door's public key) with a loop
# size of 8 (the card's loop size) produces the encryption key,
...
# machine in the cafeteria on sub-basement 7 just failed and you're
# the only one that knows how to fix it! We've already dispatched a
# reindeer to your location to pick you up."
# 
...
# bottom of the pouch is a gold coin with a little picture of a
# starfish on it.
# 
...
function inputLoad(fname)
  ...
  pk1   = parse(Int64, strip(lines[1]))
  pk2   = parse(Int64, strip(lines[2]))
